...
exports.LoadedLevels = exports.Level_SE = void 0;
...
var Level_SE = (function () {
    ...
    Level_SE.prototype.parseJSONLevel = function (lvlName) {
        var lvl = null;
        try {
    ...
    Level_SE.prototype.wallCheckLoop = function (startX, startY, dirX, dirY) {
        ...
        for (var i = 1; i < points.length; i++) {
            if ((0, MyMath_SE_1.dist)(startX, startY, closestPoint.x, closestPoint.y) >
                (0, MyMath_SE_1.dist)(startX, startY, points[i].x, points[i].y)) {
    ...
    Level_SE.prototype.quadWallPointCheck = function (sqrX, sqrY, x1, y1, x2, y2) {
        var points = [];
        if (this.walls[sqrX][sqrY][0]) {
            var pts = this.walls[sqrX][sqrY][0].simpleLineIntPoints(x1, y1, x2, y2);
            if (pts.length > 0)
                points.push.apply(points, pts);
        }
        if (sqrY + 1 < this.tilesCountY && this.walls[sqrX][sqrY + 1][0]) {
            var pts = this.walls[sqrX][sqrY + 1][0].simpleLineIntPoints(x1, y1, x2, y2);
            if (pts.length > 0)
                points.push.apply(points, pts);
        }
        if (this.walls[sqrX][sqrY][1]) {
            var pts = this.walls[sqrX][sqrY][1].simpleLineIntPoints(x1, y1, x2, y2);
            if (pts.length > 0)
                points.push.apply(points, pts);
        }
        if (sqrX + 1 < this.tilesCountX && this.walls[sqrX + 1][sqrY][1]) {
            var pts = this.walls[sqrX + 1][sqrY][1].simpleLineIntPoints(x1, y1, x2, y2);
            if (pts.length > 0)
                points.push.apply(points, pts);
        }
        return points;
    ...
    Level_SE.prototype.determineLeft = function (sqrX, sqrY, tank) {
    ...
    Level_SE.prototype.determineRight = function (sqrX, sqrY, tank) {
    ...
    Level_SE.prototype.determineTop = function (sqrX, sqrY, tank) {
    ...
    Level_SE.prototype.determineBottom = function (sqrX, sqrY, tank) {
    ...
    Level_SE.prototype.getRandomUniqueSquares = function (minX, minY, maxX, maxY, count) {
        var final = [];
        var _loop_1 = function (i) {
            var sqrX;
            var sqrY;
            do {
                sqrX = (0, MyMath_SE_1.getRandomInt)(minX, maxX);
                sqrY = (0, MyMath_SE_1.getRandomInt)(minY, maxY);
            } while (final.filter(function (value) { return (value.sqrX == sqrX && value.sqrY == sqrY); }).length > 0);
    ...
    Level_SE.prototype.getRandomSquare = function () {
        return new Geometry_SE_1.Vec2((0, MyMath_SE_1.getRandomInt)(0, this.tilesCountX - 1), (0, MyMath_SE_1.getRandomInt)(0, this.tilesCountY - 1));
    };
...
var LoadedLevels = (function () {
    function LoadedLevels() {
    }
    LoadedLevels.loadAll = function () {
        for (var lvlname in Levels) {
            var ll = LoadedLevels;
    ...
    LoadedLevels.loaded = false;
    return LoadedLevels;
}());
exports.LoadedLevels = LoadedLevels;
